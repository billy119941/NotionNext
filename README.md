...
🚀 **自动化 SEO 优化** - 系统会自动检测网站内容更新，并将新增或修改的页面提交到 Google 和 Bing 搜索引擎，提升网站在搜索结果中的收录速度。

**主要特性：**
- 🔄 **智能检测**：自动监控 sitemap.xml 变化，只提交新增或更新的内容
- 🎯 **双引擎支持**：同时支持 Google Indexing API 和 Bing IndexNow API
- ⚡ **自动化运行**：通过 GitHub Actions 每2小时自动执行一次
- 🛡️ **智能重试**：内置错误处理和重试机制，确保提交成功
- 📊 **详细报告**：提供完整的提交统计和执行日志
- 🚫 **防重复提交**：24小时冷却期避免重复提交相同URL

**快速开始：** 查看 [搜索引擎提交配置指南](./docs/USER-GUIDE.md) 了解如何启用此功能。

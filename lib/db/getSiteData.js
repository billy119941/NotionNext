...
export async function getGlobalData({
  ...
  let data = await EmptyData(pageId)
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // 站点基础信息
  const siteInfo = await getSiteInfo({ collection, block, NOTION_CONFIG })
 ...
 */
async function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const pageCover = collection?.cover
    ? await mapImgUrl(collection?.cover, collection, 'collection')
    : defaultPageCover

  // 用户头像压缩一下
  let icon = compressImage(
    collection?.icon
      ? await mapImgUrl(collection?.icon, collection, 'collection')
      : defaultIcon
  )
  // 站点网址

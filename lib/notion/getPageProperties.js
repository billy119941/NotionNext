...
export default async function getPageProperties(
  ...
  properties.pageIcon = (await mapImgUrl(value?.format?.page_icon, value)) ?? ''
  properties.pageCover = (await mapImgUrl(value?.format?.page_cover, value)) ?? ''
  properties.pageCoverThumbnail =
    (await mapImgUrl(value?.format?.page_cover, value, 'block')) ?? ''
...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  if (typeof fullPrefix === 'string' && fullPrefix.startsWith('/')) {

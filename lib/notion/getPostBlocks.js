...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('API请求超时')), 15000) // 15秒超时
      })
      ...
      console.warn('[API<<--异常]:', e.message || e)
      
      // 立即尝试从缓存获取数据
      const cacheKey = 'page_block_' + id
      const pageBlock = await getDataFromCache(cacheKey)
      if (pageBlock) {
        console.log('[使用缓存数据]', `from:${from}`, `id:${id}`)
        return pageBlock
      }
      
      // 如果还有重试次数，等待后重试
      if (retryAttempts > 1) {
        const delayTime = (4 - retryAttempts) * 2000 // 递增延迟：2s, 4s
        console.log(`[等待重试] ${delayTime}ms后重试...`)
  ...
  // 最后尝试：如果所有重试都失败，返回一个基本的错误页面结构
  console.error('[请求完全失败]:', `from:${from}`, `id:${id}`)

...
if (typeof window === 'undefined') {
  // 只在服务端导入
  try {
    const { default: converter } = require('../utils/NotionImageConverter')
    notionImageConverter = converter
    console.log('[图片映射] NotionImageConverter导入成功')
  } catch (error) {
...
const isWebPConversionEnabled = () => {
  return process.env.WEBP_ENABLE_AUTO_CONVERSION === 'true' || 
         ...
         BLOG.WEBP_ENABLE_AUTO_CONVERSION === true ||
         BLOG.WEBP_ENABLE_AUTO_CONVERSION === 'true'
}

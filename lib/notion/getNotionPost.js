...
import imageConversionTrigger from '../utils/ImageConversionTrigger'
 ...
 * @returns
 */
export async function getPost(pageId, userAgent = null) {
  ...
  // 触发图片转换（异步，不阻塞页面渲染）
  try {
    imageConversionTrigger.triggerImageConversion(pageId, blockMap, userAgent)
  } catch (error) {
    console.warn(`[文章获取] 触发图片转换失败: ${pageId}`, error)
    // 不影响正常的文章获取流程
  }
...
function getPageCover(postInfo) {
  ...
  if (pageCover && typeof pageCover === 'string') {

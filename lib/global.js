...
export function GlobalContextProvider(props) {
  ...
  // 登录验证相关 - 修复条件性hook调用问题
  const enableClerk = typeof window !== 'undefined' && process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY
  
  // 为了遵循React hooks规则，总是调用useUser，但只在需要时使用结果
  const clerkHookResult = useUser()
  
  const { isLoaded, isSignedIn, user } = enableClerk ? clerkHookResult : { 
    isLoaded: true, 
    isSignedIn: false, 
    user: false 
  }
  ...
  function switchTheme() {
    ...
    if (!THEMES || !Array.isArray(THEMES) || THEMES.length === 0) {
      console.warn('THEMES is not available or empty')
      return theme
    }

...
export async function generateRss(props) {
  ...
  if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
    console.log('Skipping RSS generation in production environment')
    return
  }
  
  // 添加安全检查
  if (!props) {
    console.warn('generateRss: props is undefined')
    return
  }
  ...
  try {
    // 检查是否在只读环境中
    if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
      console.log('Skipping RSS file write in production environment')
      return
    }

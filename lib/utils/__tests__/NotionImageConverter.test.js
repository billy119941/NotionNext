...
// Jest测试用例
describe('NotionImageConverter', () => {
  test('应该能够导入模块', () => {
    expect(notionImageConverter).toBeDefined()
  })

  test('应该能够检测图片格式', () => {
    const jpegUrl = 'https://example.com/image.jpg'
    const format = notionImageConverter._detectImageFormat(jpegUrl)
  ...
  test('应该能够验证URL有效性', () => {
    ...
    expect(notionImageConverter._isValidImageUrl(validUrl)).toBe(true)
    expect(notionImageConverter._isValidImageUrl(invalidUrl)).toBe(false)
  })
})
